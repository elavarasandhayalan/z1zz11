...
#include <iostream>
#include "GetAssayAllForPlate_CallBack.cpp"
#include "SitesByPlate_CallBack.h"
#include "SitesByPlate_CallBack.cpp"
...
extern "C" __declspec(dllexport) bool Lib_GetImage(string UserName, string Password, string DSN, string DBName, int PlateId)
{
	...
	if (ptrDB.GetHandle())
	{
		...
		//GetAssayAllForPlate_CallBack pResult(&assayIDs, &assayNames, &assayDescriptions, &createdbys);
		GetAssayAllForPlate_CallBack pResult(&assayPlateLis);
		if (MDCS_ASSAY_GetAllForPlate(ptrDB.GetHandle(), PlateId, &pResult))
		{
			...
			/*cout << "assayIDs[1] = " << assayIDs[1] << endl;
			cout << "assayIDs[2] = " << assayIDs[2] << endl;*/
		}

		//Get all sites belonging to the plate
		CArray<LONGLONG, LONGLONG> larrSiteIDs;
		SitesByPlate_CallBack pResultSites(&larrSiteIDs);
		...
		if (larrSiteIDs.GetSize()) {
			cout << "larrSiteIDs[1] = " << larrSiteIDs[1] << endl;
			lSiteID = larrSiteIDs[1];
		}
		else
		{
		...
		CArray<LONGLONG, LONGLONG> tableId;
		CArray<LPCSTR, LPCSTR> columnName;
		CArray<LPCSTR, LPCSTR> assayId;
		CArray<LPCSTR, LPCSTR> columnType;
		CArray<LPCSTR, LPCSTR> columnNameExt;
		CGetSitesByPlate_CallBack pResultMeasurements(&tableId, &columnName, &assayId, &columnType);
		if (MDCS_ASSAY_GetMeasurementsBySiteID(ptrDB.GetHandle(), lSiteID,  &pResultMeasurements))
		{
			cout << "MDCS_PLATE_GetSitesByPlate() API call Succesful .." << endl;			
		}
